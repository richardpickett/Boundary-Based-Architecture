...
| Notion exit evidence present | Notion page id(s) + exit status in fitness receipt (S8, P-020) |

**Note:** Adversarial audit operates on artifacts that have passed fitness preflight. If preflight returned `handoff_refused`, the artifact is produce-incomplete and not ready for adversarial audit. The auditor does not convert `handoff_refused` into audit FAIL — the producer fixes and resubmits. Adversarial audit refuses PASS without Notion exit evidence (P-020).
